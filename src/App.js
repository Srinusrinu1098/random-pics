...
class App extends Component {
  state = {
    ...
    random: imagesList[0].imageUrl,
    id: imagesList[0].id,
  }

  filteredItems = () => {
    const {tabId} = this.state
    const specific = imagesList.filter(each => each.category === tabId)
    return specific
  }

  changeNames = value => {
    this.setState({tabId: value, activeId: value})
  }

  clicking = value => {
    const {id} = this.state
    if (value === id) {
      const rand = Math.ceil(Math.random() * imagesList.length)
      this.setState({
        random: imagesList[rand].imageUrl,
        id: imagesList[rand].id,
      })
    }
  }

  render() {
    const {activeId, random} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        <div className="main-img-item">
          <img src={random} alt="match" className="main-img" />
              ...
              <li>
                {activeId === each.tabId ? (
                  <p
                    ...
                    onClick={() => this.changeNames(each.tabId)}
                  >
                    {each.displayText}
                  </p>
                ) : (
                ...
                {activeId === each.tabId && <hr className="hr" />}
              </li>
            ))}
          </ul>
        </div>
        <ul className="ul">
          {filteredItems.map(each => (
            <ImgItems key={each.id} value={each} clicking={this.clicking} />
          ))}
        </ul>
      </div>
    )
